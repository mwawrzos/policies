...
https://github.com/mlcommons/logging/tree/master/mlperf_logging/mllog[mlperf_logging.mllog package].

For Inference, the results logs must have been produced by the https://github.com/mlperf/inference/tree/master/loadgen[standard load generator]. Power information may be appended using the standard power information appending script [TODO link or remove].
...
Submitters must run the https://github.com/mlcommons/logging/tree/master/mlperf_logging/package_checker[package_checker]
and the https://github.com/mlcommons/logging/tree/master/mlperf_logging/system_desc_checker[system_description_checker] tools
to verify that their submission achieves a basic level of compliance with a subset of the MLPerf rules.
If compliance tools identifies a potential issue with the submission, the onus is on the submitter to provide an adequate explanation to the results review committee.
